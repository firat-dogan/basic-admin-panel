...
check();
$id=$_GET['id'];
$result=select("Crew", "WHERE id={$id}");
$row= mysql_fetch_array($result);
if(isset($_POST['save']))
{
	$name=$_POST['name'];
	$job = $_POST['job'];
	$db = update("Crew","name='{$name}',job='{$job}'", "id={$id}");
	if($db)
    ...
    <main role="main" class="container pt-4">
      <div class="row">
       <div class="col-md-12">
		  ...
		  <input type="text" value ="<?= $row['name']?>" class="form-control" id="" placeholder="" name = "name">
                </div>
              </div>
		  ...
		  <input name="job" type="text" value="<?= $row['job']?>" class="form-control" >
                </div>
              </div>

              <div class="col-md-12">
                  <button type="submit" name= "save" class="btn btn-success">Kaydet</button>
                <a href="ekibimiz.php" class="btn btn-light">İptal</a>
